...
function PagePlayer(oConfigOverride) {
  ...
  sm.url = '/swfs/'; // path to directory containing SM2 SWF
  ...
  this.events = {
    ...
    play: function() {
      ...
      // CALLBACK TO INCREMENT COUNTER HERE!!
    },
  ...
  // THIS IS THE FUNCTION THAT YOU WOULD LIKE TO MODIFY
  this.handleClick = function(event) {
    // a sound (or something) was clicked - determine what and handle appropriately
    // Handle the right click from IE
    if (event.button == 2) {
      if (!pl.config.allowRightClick) pl.stopEvent(event);
    ...
    var o = element; // just in case
    if (self.dragActive) self.stopDrag(); // to be safe
    if (self.withinStatusBar(element)) {
      // self.handleStatusClick(e);
      return false;
    }
    if (element.nodeName.toLowerCase() != 'a') {
      element = self.getParentByNodeName(element,'a');
    }
    if (!element) {
      // not a link
      return true;
    ...
    var url = element.getAttribute('sound');

    // var sURL = element.readAttribute('href');
    if (!url || !url.match(/.mp3/i)) {
      if (isIE && element.onclick) {
        return false; // IE will run this handler before .onclick(), everyone else is cool?
      }
      return true; // pass-thru for non-MP3/non-links
    }
    
    // var soundURL = element.href;
    var soundURL = url;
    var thisSound = self.getSoundByObject(element);
    if (thisSound) {
      ...
      element.parentNode.appendChild(oControls);
      element.parentNode.appendChild(document.getElementById('spectrum-container'));
      self.soundsByObject[element.rel] = thisSound;
      // tack on some custom data
      thisSound._data = {
        oLink: element, // DOM reference within SM2 object event handlers
        oLI: element.parentNode,
        oControls: self.getElementsByClassName('controls','div',element.parentNode)[0],
        oStatus: self.getElementsByClassName('statusbar','div',element.parentNode)[0],
        oLoading: self.getElementsByClassName('loading','div',element.parentNode)[0],
        oPosition: self.getElementsByClassName('position','div',element.parentNode)[0],
        oTimingBox: self.getElementsByClassName('timing','div',element.parentNode)[0],
        oTiming: self.getElementsByClassName('timing','div',element.parentNode)[0].getElementsByTagName('div')[0],
        oPeak: self.getElementsByClassName('peak','div',element.parentNode)[0],
        oGraph: self.getElementsByClassName('spectrum-box','div',element.parentNode)[0],
        nIndex: self.getSoundIndex(element),
  ...
  this.init = function() {
    ...
    for (var i=0; i<oLinks.length; i++) {
      ...
      try{
        var str = oLinks[i].getAttribute('sound');        
      }
      catch(e){
...
soundManager.onload = function() {
  ...
  soundManager.defaultOptions.volume = 40;
  pagePlayer.initDOM();
}